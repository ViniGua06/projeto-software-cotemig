...
class UserController {
  ...
  createUser = async (req: Request, res: Response) => {
    try {
      ...
      const token = await jwtService.createToken(userInserted.toString());
  ...
  goToChurch = async (req: Request, res: Response) => {
    try {
      const id = req.params.id;
      const { church_id, role } = req.body;

      await repository.goToChurch(church_id, parseInt(id), role);
  ...
  enterChurch = async (req: Request, res: Response) => {
    try {
      ...
      console.log(user_id);
      await repository.enterChurch(user_id, token);
